...
client.on('interactionCreate', async interaction => {
    ...
    // --- คำสั่ง /card ---
    if (commandName === 'card') {
        // 1. เช็คว่าเป็นแอดมิน (ตรวจสอบจาก Role ID ใน ENV) หรือเป็นเจ้าของเซิร์ฟเวอร์
        const isAdmin = ADMIN_ROLE_IDS.some(roleId => interaction.member.roles.cache.has(roleId)) || interaction.member.id === interaction.guild.ownerId;
        
        // 2. ถ้าไม่ใช่แอดมิน ให้ซ่อนข้อความ (ephemeral = true)
        const isHidden = !isAdmin;

        // 3. แนบสถานะซ่อนไปตั้งแต่ตอนเริ่มโหลด
